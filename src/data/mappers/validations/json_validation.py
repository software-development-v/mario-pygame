...
def validate_field_type(
    value: Any, expected_type: type, field_name: str, positive: bool = False
):
    if not isinstance(value, expected_type):
        raise ValueError(
            f"Invalid '{field_name}' field: must be a {expected_type.__name__}"
        )
    if positive and value <= 0:
        raise ValueError(
            f"Invalid '{field_name}' field: must be a positive {expected_type.__name__}"
        )
...
def validate_background(background: Dict[str, Any]):
    ...
    if background["type"] == "COLOR":
        validate_field_type(
            background.get("resource"), list, "background['resource']"
        )
        if len(background["resource"]) != 3:
            raise ValueError(
                "Invalid 'background' field: 'resource' must be a list with 3 elements"
            )
        if any(value < 0 or value > 255 for value in background["resource"]):
            raise ValueError(
                "Invalid 'background' field: 'resource' must be a list with values between 0 and 255"
            )
    else:
        raise ValueError(
            "Invalid 'background' field: 'type' must be a valid background type"
        )
...
def validate_start_player_position(start_player_position: Dict[str, Any]):
    ...
    validate_field_type(
        start_player_position.get("x"),
        int,
        "start_player_position['x']",
        positive=True,
    )
    validate_field_type(
        start_player_position.get("y"),
        int,
        "start_player_position['y']",
        positive=True,
    )
...
def validate_list_of_entitie_dicts(lst: Dict[str, Any], field_name: str):
    ...
    for idx, item in enumerate(lst, start=1):
        if not isinstance(item, dict):
            raise ValueError(
                f"Invalid '{field_name}' field: element at index {idx} must be a dictionary"
            )
        else:
