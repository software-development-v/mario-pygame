...
class LevelMetricsRenderer:
    ...
    def render(self,name:str, time: int, score: int, coins: int, world: int=1,level:int=1) -> None:
    ...
    def set_status_bar(self) -> None:
        ...
        self.set_message_box(f"{self.character_name}\n{format_score}", x_base, self.status_bar_y_pos)
        self.set_message_box(f"\nx{self.coins}", x_base + (1 * spacing), self.status_bar_y_pos)
        self.set_message_box(f"WORLD\n {self.world}-{self.level}", x_base + (2 * spacing), self.status_bar_y_pos)
        self.set_message_box(
            f"TIME\n {format_time}", x_base + (3 * spacing), self.status_bar_y_pos
        )

        self.surf.blit(
                    transform.scale(COIN_1,(self.icons_size,self.icons_size)),
                    (x_base + (1 * spacing)-self.icons_size*2.3, self.status_bar_y_pos+self.icons_size)
                )
