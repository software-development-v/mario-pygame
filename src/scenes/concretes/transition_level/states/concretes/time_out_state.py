...
class TimeoutState(LevelState):
    ...
    def render(self, screen: Surface) -> None:
        ...
        if self.time <= 0:
            self.level_manager.set_current_time(self.level_manager.get_start_time())
           
            if self.level_manager.get_lifes() == 0:
                self.change_state(GameOverState(self.level_manager, self.game_data, self.change_state))
            else:
                self.change_state(LevelStatusState(self.level_manager, self.game_data, self.change_state))
