...
from src.scenes.concretes.level.level_metrics_renderer import (
    LevelMetricsRenderer,
)
...
class TransitionLevelSceneRender(Render):
    ...
    def render(self) -> None:
        ...
        if self.time > 0:
            ...
            message, message_rect = get_centered_message(
                "TIME OUT", text_color=WHITE_COLOR
            )
    ...
    def render_status_game(self):
        ...
        if self.__level_manager.get_lifes() == 0:
            message, message_rect = get_centered_message(
                "GAME OVER", text_color=WHITE_COLOR
            )
            self._screen.blit(message, (message_rect.x, message_rect.y))
            ...
            message, message_rect = get_centered_message(
                f"WORLD {world}-{level}", text_color=WHITE_COLOR
            )
            lifes, lifes_rect = get_centered_message(
                f"X {self.__level_manager.get_lifes()}", text_color=WHITE_COLOR
            )
