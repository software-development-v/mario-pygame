...
class ModeSelectionSceneRender(IRender):
    def __init__(self) -> None:
        ...
        self.button_quit = Button(self.menu_options_left[0], pygame.font.Font(GAME_FONT, 30), (200, 140), ORANGE_COLOR, BLACK_COLOR)

        option_height = 100
        self.buttons_center: List[Button] = []
        for index, option in enumerate(self.menu_options_center):
            position = (screen_width // 2, (screen_height // 2 - (len(self.menu_options_center) // 2 * option_height) + index * option_height) - 50)
            button = Button(option, pygame.font.Font(GAME_FONT, 40), position, ORANGE_COLOR, BLACK_COLOR)
            self.buttons_center.append(button)
        self.button_options = Button(self.menu_options_right[0], pygame.font.Font(GAME_FONT, 30), (1400 , 140), ORANGE_COLOR, BLACK_COLOR)
    ...
    def render(
        ...
        for index, button in enumerate(self.buttons_center):
            button.render(self._screen, self.selected_section == 1 and index == self.selected_option)
    ...
    def handle_mouse_event(self, mouse_pos: Tuple[int, int]) -> bool:
        if self.button_quit.is_hovered(mouse_pos) and self.selected_section == 0:
            self.selected_option = 0
            return True

        if self.button_options.is_hovered(mouse_pos) and self.selected_section == 2:
            self.selected_option = 0
            return True
